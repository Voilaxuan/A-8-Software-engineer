...
def trace(target):
    ...
    try:
        ...
        print("targe",target)
        target_urllist, iplist, collect_dirs, collect_ports, subdomain ,dns_content, different_dsn ,hashid = site_info_collect.runscan(target)
        print("target_urllist",target_urllist)
        # urla = checkurl(target)
        # id = hashlib.md5()
        # id.update(urla)
        # siteid = id.hexdigest()A
