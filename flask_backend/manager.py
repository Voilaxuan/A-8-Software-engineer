...
def get_info():
    conn=mysql.connector.connect(user='root',password='123456',host='localhost',database='TDScan',auth_plugin='caching_sha2_password')
    cur=conn.cursor()
...
def dellog(id):
    ...
    cur.close()
    # return render_template("manager.html", result_infos=result_infos)
    conn2=mysql.connector.connect(user='root',password='123456',host='localhost',database='TDScan',auth_plugin='caching_sha2_password')
    cur2=conn2.cursor()
