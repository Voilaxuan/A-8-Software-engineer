...
from cat.core_engine.php.parser import anlysis_params as php_anlysis_params
...
class CAST(object):
    ...
    def functions(self):#get all functions
        # grep = Tool().grep
    ...
    def block_code(self, block_position):#get block code
        if block_position == 2:
    ...
    def is_controllable_param(self):
        ...
        param_name = re.findall(self.rule, self.code)#在code里匹配与正则表达式rule相符的字符串
        ...
        for param_name in params:
            try:
                ...
                # all is string
                regex_string = self.regex[self.language]['string']#所有的字符串
                string = re.findall(regex_string, param_name)
                #字符串中包含变量或不包含
                if len(string) >= 1 and string[0] != '':
                    regex_get_variable_result = re.findall(self.regex[self.language]['variable'], param_name)#危险变量
                ...
                if self.language == 'php':
                    ...
                    if _is_co == 1:
                        ...
                        return False, _is_co, tuple([_is_co, _cp]), chain#列表转换为元祖
                    else:
                        continue
