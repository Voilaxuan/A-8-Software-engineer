...
from cat.pretreatment import ast_object
from cat.internal_defines.php.functions import function_dict as php_function_dict
                ...
                if isinstance(node.expr, php.Array):
                    for p_node in node.expr.nodes:
                        if p_node.key == param_expr:
                            if isinstance(p_node.value, php.ArrayOffset):  # 如果赋值仍然是数组，先经过判断在进入递归
...
def parameters_back(param, nodes, function_params=None, lineno=0,
    ...
    if hasattr(param, "name"):#判断param是否包含name
        # param_name = param.name
        ...
        if isinstance(node, php.Assignment) and param_name == get_node_name(node.node):  # 回溯的过程中，对出现赋值情况的节点进行跟踪
            ...
            if param_name == param_node and not isinstance(param_expr, list):  # 找到变量的来源，开始继续分析变量的赋值表达式是否可控
                ...
                if is_co != 1 and is_co != 3:
                    is_co, cp = is_sink_function(param_expr, function_params)#判断此函数是否为危险函数

                if is_co == -1 and isback is True:#isback：是否需要返回该值
                    cp = param_expr
            ...
            if is_co is not 1 and node.elseifs != []:  # elseif可能有多个，所以需要列表
                ...
                for node_elseifs_node in node.elseifs:#elseif里可能是代码块，同样需要递归
...
def anlysis_params(param, file_path, lineno, vul_function=None, repair_functions=None, controlled_params=None,
    ...
    while isinstance(param, php.Variable):#判断param是否是php.Variable类型
        param = param.name
