...
import TDScanner.config as config
import subprocess
...
# 随机返回一个userAgent
def get_user_Agent():
    index = random.randint(0, len(config.User_Agents) - 1)
...
def is_domain_ping(url):
    try:
        ...
        if isinstance(url_info, tuple):
            ...
            host = str(url_info)
        pings = subprocess.run(['ping','-c','1',host],capture_output= True)
        if pings == 0:
            return 1
        else:
            return -1
...
def get_subdomain_dict():
    try:
        subdomain_list = []
        path = os.getcwd() + config.SUB_DOMAIN_DIR
...
def get_next_subdomain_dict():
    subdomain_list = []
    path = os.getcwd() + config.NEXT_SUB_DOMAIN_DIR
    with open(path, "r") as f:
        content = f.readlines()
...
def load_dns_servers():
    dns_servers = []
    path = os.getcwd() + config.DNS_SERVER_DIR
    with open(path, "r") as f:
        for line in f:
            server = line.strip()
