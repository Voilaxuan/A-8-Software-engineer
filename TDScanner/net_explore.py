#!/usr/bin/env python
# -*- encoding: utf-8 -*-
from TDScanner.libs.utils import Queue
import socket
import threading
...
def getdata(host,port):
    ...
    #print "aimurl:"+aimurl
    try:
        data = urllib.request.urlopen(aimurl, timeout=3)
        ...
        try:
            LOGFILE.write(
                "http://" + host + ":" + port + "/" + " #" + retitle[0].decode("utf-8").encode(os_char) + "#" + deip[
                    0] + '\n')
        finally:
            LOGFILE.flush()
    except:
...
def netmain(addr):
    ...
    if os.path.exists(path) and os.path.getsize(path) != 0 :
        ...
        # ipcs = int(ipn[2]) - 1
        # ipce = int(ipn[2]) + 1
        startip = ipn[0] + '.' + ipn[1] + '.' + str(ipn[2]) + '.' + '1'
        endip = ipn[0] + '.' + ipn[1] + '.' + str(ipn[2]) + '.' + '255'
        iplist = ip_range(startip, endip)
        bThread(iplist)

#netmain('1.189.195.98')
